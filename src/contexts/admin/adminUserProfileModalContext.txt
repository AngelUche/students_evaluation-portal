...
export const UserProfileModalContext = createContext(
  {} as UserProfileModalContextProps
);

// Create the Context Provider for the UserProfileModal
export function UserProfileModalContextProvider({
  children,
}: UserProfileModalContextProviderProps) {
  const [showUserProfile, setShowUserProfile] =
    useState<ShowUserProfileInterface>({ status: true, id: undefined });
  ...
  return (
    <UserProfileModalContext.Provider value={values}>
      {children}
