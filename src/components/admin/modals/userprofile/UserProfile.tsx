...
function UserProfileModal({ id }: UserProfileModalProps) {
  ...
  return (
    <div
      className="w-[350px] bg-white rounded-[10px] overflow-hidden shadow-xl"
        ...
        <div className="flex flex-col gap-3 py-6 px-5">
          <div className="flex justify-between items-start">
            ...
            <div className="h-full flex flex-col gap-1 justify-between items-end">
              <p className="text-[10px] text-gray-500 font-mono">
                Last login: 07 Aug 2023, 14.04PM
              </p>
              {User!.classDesignation && (
                <p className="text-[12px] text-gray-800 font-mono">
                  Class: {User!.classDesignation}
                </p>
              )}
              <a
                href={`mailto:${User!.email}`}
                className="text-[12px] text-blue-700 font-mono"
              >
                {User!.email}
              </a>
            </div>
          ...
          <div className="flex flex-col gap-5">
            <div className="flex gap-2">
              <p className="text-sm py-2 text-gray-900 uppercase w-full border-b-[1px] border-gray-300">
                {User!.name}
              </p>
            </div>
            <div>
              <p className="text-sm text-gray-700 pb-2 border-b-[1px] border-gray-300">
                {User!.phoneNumber}
              </p>
            </div>
          </div>
          <button
            className="p-1 py-3 mt-3 rounded bg-[#038f4d] text-white hover:bg-[#055c32]"
            onClick={(e) => {
              e.stopPropagation();
